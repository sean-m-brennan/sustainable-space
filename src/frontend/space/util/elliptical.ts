...
import {angleBetweenVectors, array2vector, rotateVector, X_AXIS, Y_AXIS, Z_AXIS} from './coordinates'
import {SatelliteState} from "../planet/satellite"
...
export const ellipticalPropagation = (satData: SatelliteState, speed: number, center: ArrayVector, refDir: ArrayVector, refAngle: ArrayVector) => {
    const sat = {...satData}  // copy
    sat.travelled += speed * (sat.planetOrbit.orbitalSpeed / (2 * Math.PI)) * 100.0
    if (sat.travelled >= (2 * Math.PI))
        sat.travelled -= (2 * Math.PI)

    sat.position = ellipticalOrbit(sat.orbit.semiMajorAxis, sat.orbit.eccentricity,
        sat.orbit.inclination, sat.orbit.raan, center, refDir, refAngle,
        sat.travelled)
